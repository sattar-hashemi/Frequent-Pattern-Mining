function [ranked,weight] = online_relieff(X,Y,K,weight_old,Continuous)
...
weight = NaN(1,p);
    ...
    weight(accepted) = RelieffClass(X,C,classProb,numUpdates,K,distFcn,dist1D,sigma,weight_old);
...
function attrWeights = RelieffClass(scaledX,C,classProb,numUpdates,K,...
    distFcn,dist1D,sigma,WeigthOld)
...
for i = 1:numUpdates
    ...
    % Inner loop to update weights for each attribute:
        ...
        dH = diffH(1,scaledX,thisObs,Hits,dist1D,sigma)/numUpdates;
        dM = diffM(1,scaledX,thisObs,Misses,dist1D,sigma,classProb)/numUpdates;
        attrWeights = WeigthOld - dH + dM;
